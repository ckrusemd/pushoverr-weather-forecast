...
df.score %>% 
  ...
  geom_text(aes(label=paste0(ifelse(metric %in% c("uvi","temp"),round(value,1),round(value,0)))),color="black") +
  theme_bw() +
...
gain_sunrise = df.dates.next_week %>% 
  ...
  dplyr::mutate(change=glue("{change$minute}:{str_pad(abs(change$.Data),2,pad='0')}")) %>%
...
total_gain = df.dates.next_week %>% 
  ...
  filter(min(EventTime)==EventTime) %>%
  ungroup() %>% 
  dplyr::summarize(change=sum(abs(change))) %>% 
  dplyr::mutate(change=lubridate::seconds_to_period(change)) %>% 
  dplyr::mutate(change=glue("+{change$minute}:{str_pad(abs(change$.Data),2,pad='0')}")) %>% 
  pull(change)
