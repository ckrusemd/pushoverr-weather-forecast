...
PUSHOVER_APPKEY = Sys.getenv("PUSHOVER_APPKEY")
PUSHOVER_USERKEY = Sys.getenv("PUSHOVER_USERKEY")
...
missing_env_vars = names(required_env_vars)[trimws(required_env_vars) == ""]
if (length(missing_env_vars) > 0) {
  stop(glue("Missing required environment variable(s): {paste(missing_env_vars, collapse = ', ')}"), call. = FALSE)
}

pushoverr::set_pushover_app(token = PUSHOVER_APPKEY)
pushoverr::set_pushover_user(user = PUSHOVER_USERKEY)
...
weather_config = list(
  locations = list(
    primary = list(lat = 55.771979, lon = 12.494786),
    primary_suncalc = list(lat = 55.7715024, lon = 12.4932282),
    aarhus = list(lat = 56.1572, lon = 10.2107)
  ),
  timezones = list(
    openweather = "Europe/Copenhagen",
    suncalc = "CET"
  )
)
```
...
convert_timestamp = function(timestamp) {
  return( as.POSIXct( timestamp ,tz = weather_config$timezones$openweather,origin="1970-01-01") )
}
```
...
```{r}
fetch_openweather_onecall = function(lat, lon, api_key) {
  response = httr::GET(
  ...
  if (response_status != 200) {
    response_body = httr::content(response, as = "text", encoding = "UTF-8")
    stop(glue("OpenWeather API request failed with status {response_status}: {response_body}"), call. = FALSE)
  }
  ...
  parsed_response = tryCatch(
    ...
    error = function(e) {
      stop(glue("Failed to parse OpenWeather API JSON response: {conditionMessage(e)}"), call. = FALSE)
    }
  )

  required_fields = c("current", "daily", "hourly", "minutely")
  missing_fields = setdiff(required_fields, names(parsed_response))
  if (length(missing_fields) > 0) {
    stop(glue("OpenWeather API response missing required field(s): {paste(missing_fields, collapse = ', ')}"), call. = FALSE)
  }
...
lat = weather_config$locations$primary$lat
lon = weather_config$locations$primary$lon

res_json = fetch_openweather_onecall(lat = lat, lon = lon, api_key = OPENWEATHERMAP_APIKEY)
...
df.scores = res_json_hourly %>% 
  filter(dt<=ymd_hms(paste0(Sys.Date()," 00:00:00"),tz = weather_config$timezones$openweather)+days(1)) %>% 
...
```{r}
current_dt = as.POSIXct(format(Sys.time()),tz=weather_config$timezones$suncalc)
current_date = as.Date(Sys.Date(),tz=weather_config$timezones$suncalc)
...
df.dates.next_week = suncalc::getSunlightTimes(date = seq_next_14,
                            lat = weather_config$locations$primary_suncalc$lat,
                            lon = weather_config$locations$primary_suncalc$lon,
                            tz = weather_config$timezones$suncalc,
                            keep = event_order) %>% 
...
df.dates.next_3_months =
  suncalc::getSunlightTimes(date = seq_next_3_months,
                            lat = weather_config$locations$aarhus$lat,
                            lon = weather_config$locations$aarhus$lon,
                            tz = weather_config$timezones$suncalc,
                            keep = c("solarNoon", 
                                     "sunrise", 
                                     "sunset", 
  ...
  group_by(date) %>% 
  dplyr::mutate(EventTime=as.numeric(difftime(EventTime,ymd_hms(paste0(date," 00:00:00"),tz = weather_config$timezones$openweather),units = "hours")))
...
df.dates =
  ...
  group_by(EventName) %>% 
  dplyr::mutate(StartTime=as.POSIXct(StartTime,origin="1970-01-01"),tz=weather_config$timezones$suncalc) %>% 
  dplyr::mutate(StartTime=ymd_hms(paste0(year(EventTime),"-",month(EventTime),"-",day(EventTime)," ",hour(StartTime),":",minute(StartTime),":",second(StartTime)),tz=weather_config$timezones$suncalc)) %>% 
  dplyr::mutate(EventTime=as.POSIXct(EventTime,tz=weather_config$timezones$suncalc)) %>% 
