...
gain_sunset = df.dates.next_week %>% 
  ...
  dplyr::mutate(change=-1*(difftime(EventTime,lag((EventTime+hours(24))),units = "secs"))) %>% 
  dplyr::mutate(change=-1*as.numeric(change)) %>% 
  # dplyr::mutate(change=sprintf("+%02d:%02d", change %% 3600 %/% 60,  change %% 60 %/% 1)) %>% 
  dplyr::mutate(change=sprintf("+%02d:%02d", change %/% 60,  change %% 60 %/% 1)) %>% 
  filter(EventTime>=Sys.Date()) %>% 
  filter(min(EventTime)==EventTime) %>% 
  pull(change)
...
msg_bike = tryCatch(expr= {
  ...
  filter(hour_==7) %>%
  slice(c(1)) %>% 
  dplyr::select(feels_like,pop,uvi,
                feels_like_score,pop_score,uvi_score)
  feels_like = bike_score$feels_like
  pop_ = bike_score$pop
  uvi_ = bike_score$uvi
  if (length(temp_)==0) stop();
  glue("Bike: {scales::number(feels_like,accuracy=0.1)} C, PoP: {pop_}%, UV: {scales::number(uvi_,accuracy=0.1)}")
},error=function(e) {
  glue("Bike: N/A")
})
...
msg_bikehome = tryCatch(expr= {
  ...
  uvi_ = bike_score$uvi
  if (length(temp_)==0) stop();
  glue("Bike home: {scales::number(feels_like,accuracy=0.1)} C, PoP: {pop_}%, UV: {scales::number(uvi_,accuracy=0.1)}")
},error=function(e) {
  glue("Bike home: N/A")
})
